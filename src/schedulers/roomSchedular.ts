...
const roomCategoryRepository = new Roomcategory();
...
const extendCategoryAvailability = async (categoryData: { roomCategoryId: number, latestDate: Date }): Promise<void> => {
    try {
        ...
        const roomCategory = await roomCategoryRepository.getById(roomCategoryId);
        if (!roomCategory) {
        ...
        // Add job to queue
        await addRoomGenerationToQueue(jobData);
